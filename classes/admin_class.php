...
/* ---------------------- Admin Login Check ----------------------------------- */
    ...
    public function admin_login_check($data) {
        ...
        try
       {
          $stmt = $this->db->prepare("SELECT * FROM users WHERE username=:uname AND password=:upass LIMIT 1");
...
/*----------- add_new_user--------------*/
	...
	public function add_new_user($data){
		...
		try{
			...
			$sqlUsername = "SELECT username FROM users WHERE username = '$user_username' ";
...
/* ---------update_user_data----------*/
	...
	public function update_user_data($data, $id){
		...
		$user_email = $this->test_form_input_data($data['em_email']);
		try{
			$update_user = $this->db->prepare("UPDATE users SET fullname = :x, username = :y, email = :z WHERE user_id = :id ");
...
/* ------update_user_password------------------*/
	...
	public function update_user_password($data, $id){
		...
		try{
			$update_user_password = $this->db->prepare("UPDATE users SET password = :x WHERE user_id = :id ");
...
/* -------------admin_password_change------------*/
	...
	public function admin_password_change($data, $id){
		...
		try{
			...
			$sql = "SELECT * FROM users WHERE user_id = '$id' AND password = '$admin_old_password' ";
			...
			if(empty($all_error)){
				$update_admin_password = $this->db->prepare("UPDATE users SET password = :x WHERE user_id = :id ");
	...
	public function add_new_task($data){
		...
		try{
			$add_task = $this->db->prepare("INSERT INTO project_info (t_title, t_description, t_start_time, 	t_end_time, t_user_id) VALUES (:x, :y, :z, :a, :b) ");
		...
		public function update_project_info($data, $task_id, $user_role){
			$task_title  = $this->test_form_input_data($data['task_title']);
			$task_description = $this->test_form_input_data($data['task_description']);
			$t_start_time = $this->test_form_input_data($data['t_start_time']);
			$t_end_time = $this->test_form_input_data($data['t_end_time']);
			...
			if($user_role == 1){
				...
				$sql = "SELECT * FROM project_info WHERE task_id='$task_id' ";
			...
			try{
				$update_task = $this->db->prepare("UPDATE project_info SET t_title = :x, t_description = :y, t_start_time = :z, t_end_time = :a, t_user_id = :b, status = :c WHERE task_id = :id ");
				...
				$_SESSION['Task_msg'] = 'Project Update Successfully';
